...
get_build_report() {
  ...
  elif [ "$CIRCLE_JOB" == 'build_docker_image' -a  "$STATUS" == 'success' ]; then

    MESSAGE_TEXT="Build Docker image Phase Passed! :smiley:"
    COLOR="good"

  elif [ "$CIRCLE_JOB" == 'build_docker_image' -a  "$STATUS" == 'fail' ]; then

    MESSAGE_TEXT="Build Docker image Phase Failed :scream:"
    COLOR="danger"
    REBUILD_URL="https://circleci.com/actions/retry/github/andela/mrm_api/${CIRCLE_BUILD_NUM}"
    ACTION_BUTTON="$(echo \
          "{\"type\": \"button\", \"text\": \"Rebuild\", \"url\": \"${REBUILD_URL}\"}", \
